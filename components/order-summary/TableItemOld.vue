    ...
    <div class="rounded-md shadow-m p-[15px]">
        <div class="flex items-start justify-between mb-10">
            <div class="relative flex items-start">
                ...
                <div
                    v-if="item.discount"
                    class="hidden absolute right-0 top-[3px] translate-x-10 bg-rose-500 rounded-[25px] px-1.5 py-0.5 text-xs font-extrabold text-white md:flex"
                >
                    {{ item.discount }}%
                </div>
            </div>
            ...
            <div class="relative flex flex-col gap-[5px] md:flex-row md:items-center md:gap-[30px]">
                ...
                <div class="flex items-center text-xs font-Inter">
                    ...
                    <span class="flex items-end text-sm font-medium leading-snug md:text-base md:items-center">
                        <span class="" :class="[item.discount ? 'bg-green-500 mr-[5px]' : '']"> $120,00 </span>
                ...
                <div
                    v-if="item.discount"
                    class="absolute left-0 -top-[15px] -translate-y-full bg-rose-500 rounded-[25px] px-1.5 py-0.5 text-xs font-extrabold text-white md:hidden"
                >
                    {{ item.discount }}%
                </div>
            </div>
