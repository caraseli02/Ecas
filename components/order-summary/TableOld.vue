<template>
    <div class="mb-10 lg:mb-[50px]">
        ...
        <div class="grid grid-cols-1 gap-5 mb-[30px] md:gap-[15px]">
            <OrderSummaryTableItem v-for="(item, index) in filteredItems" :key="index" :item="item" class="xl:hidden" />
        ...
        <div class="flex flex-col items-center w-full md:flex-row md:justify-between">
            <label class="flex items-center mb-[30px] md:mb-0">
                <span class="text-sm font-medium flex-shrink-0 mr-2.5"> Products on page </span>
                <div class="relative">
                    <button
...
const filteredItems = computed(() => {
    return items.value.slice((atPage.value - 1) * perPage.value, (atPage.value - 1) * perPage.value + perPage.value);
});
