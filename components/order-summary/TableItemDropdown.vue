  ...
  <div class="bg-[#FFF] rounded-b-lg flex flex-col gap-4 py-4">
      ...
      <div class="xl:pl-0 px-4">
        <div class="grid grid-cols-1 xl:grid-flow-col xl:gap-[16px] gap-2">
          <div class="flex flex-row justify-between xl:gap-4 items-center xl:flex-col">
                        <span
                            class="text-[#222] text-center text-sm font-normal leading-6 xl:gap-4 px-0 xl:px-4 whitespace-nowrap"
                        >Unit Price</span
                        >
            ...
            <div v-if="item.discount?.value" class="flex flex-col text-center h-[36px] justify-center">
                            <span
                                ...
                                class="line-through text-[#222] text-sm font-normal leading-5"
                            >$ {{ item.initialUnitPrice }}</span
                            >
            ...
            <div v-else class="flex flex-col text-center h-[36px] justify-center">
              <span class="text-[#222] text-sm font-normal leading-5">$ {{ item.subtotal }}</span>
            </div>
          </div>
          ...
          <div class="flex flex-row justify-between items-center xl:flex-col xl:gap-4">
            <span
                class="text-[#222] text-center text-sm font-normal leading-6 xl:gap-4 px-0 xl:px-4">Tax (VAT 19%)</span>
            <div class="flex flex-col text-center h-[36px] justify-center">
              <span class="text-[#222] text-sm font-normal leading-5">$ {{ taxPrice }}</span>
            </div>
          </div>
          <div class="flex flex-row justify-between items-center xl:flex-col xl:gap-4">
            <span class="text-[#222] text-center text-sm font-normal leading-6 xl:gap-4 px-0 xl:px-4">Subtotal</span>
            <div class="flex flex-col text-center h-[36px] justify-center">
              <span class="text-[#222] text-sm font-normal leading-5">$ {{ subtotal }}</span>
            </div>
          </div>
        </div>
      </div>
    ...
    <div class="relative px-4 flex flex-row justify-between xl:justify-start xl:gap-6">
      <button
          ...
          @click="showPackagingDetails = !showPackagingDetails">
      ...
      <Transition name="fade-bottom">
        ...
        >
          ...
          <p class="text-[#222] text-xs font-normal leading-5">
            A tube is a rigid extruded plastic package designed to fit the profile of the part and protect the leads.
            Tubes ship
            with the exact number of parts ordered and a rubber plug or plastic peg in each end to prevent the parts
            from
            sliding out.
          </p>
        </div>
  ...
  <Teleport to="body">
    <Transition name="slide-from-top">
      <LayoutFavoritesModalsDelete
          v-if="deleteItem" :products="[item as any]" :delete-from-cart="true"
