...
v-if="stockItem"
          ...
          <span
v-else
                class="block truncate text-[#FA4B4B] text-sm font-normal leading-6 self-center justify-self-center">{{
              item.id
            }}</span>
        </div>
...
v-if="stockItem && item.discount.value"
      ...
      <div>
        ...
        <ChevronDownIcon
class="w-5 h-5 flex-shrink-0 rounded-full transition duration-300 xl:hidden flex"
                         :class="[expanded ? 'rotate-180 text-[#007FFF]' : 'text-gray-300']"/>
      </div>
    </div>
    <Transition name="expand">
      <OrderSummaryTableItemDropdown
v-if="expanded" ref="tab" :item="item" :short-stock="shortStock"
                                     :stock-item="stockItem" :liked="liked"
                                     class="item"/>
    </Transition>
  </div>
</template>
