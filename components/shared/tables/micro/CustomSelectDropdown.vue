  ...
  <div class="absolute z-10 -translate-x-full grid grid-cols-1 gap-1 rounded-lg bg-white p-3 shadow-m" :style="{
    ...
    <div class="overflow-y-auto scrollbar-thin max-h-[304px]">
      <button v-for="item in items"
        ...
        :class="[selectedItem === item.label ? 'text-blue bg-[#F2F2F2]' : '']" @click="handleSelection(event, item)">
        <component v-if="item.iconType" :is="item.iconType" class="w-6 h-6 mr-3 text-current" />
