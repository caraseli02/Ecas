<template>
    <div class="absolute z-10 grid grid-cols-1 gap-1 rounded-lg bg-white p-3 w-[195px] shadow-m -translate-x-full"
        :class="[index > 5 ? '-translate-y-[calc(100%+42px)]' : '']" :style="{
            top: dropdownTop + 'px',
            left: dropdownLeft + 'px',
        }">
        <button v-if="profileButton"
...
export default defineComponent({
    name: 'ThreeDotMenu',
    props: ['index', 'dropdownTop', 'dropdownLeft', 'profileButton', 'profile',
        'profileText', 'documentButton', 'documentText', 'settingsButton', 'settingsText',
        'deactivateButton', 'deactivateText', 'trashButton', 'trashText', 'invoiceButton', 'invoiceText', 'editButton', 'editText',
        'active',
    ],
    components: {
        OrdersIcon,
