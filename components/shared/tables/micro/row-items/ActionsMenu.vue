<template>
  ...
  <template v-else>
    <Tooltip :position="index === 0 ? 'bottom' : 'top'" theme="black">
      <button v-if="documentButton" @click="documentClicked"
    ...
    <Tooltip :position="index === 0 ? 'bottom' : 'top'" theme="black">
      <button v-if="invoiceButton" @click="invoiceClicked"
        class="flex text-gray-300 transition-colors duration-300 hover:text-blue">
      ...
      <button v-if="downloadButton" @click="downloadClicked" class="flex text-gray-300 transition-colors duration-300 hover:text-blue">
...
export default defineComponent({
  name: 'ActionsMenu',
  props: ['loading', 'index', 'documentButton', 'invoiceButton', 'downloadButton', 'threeDotButton', 'txtDocument', 'txtInvoice', 'txtDownload'],
  components: {
    TooltipVue,
    SkeletonLoader,
    DocumentIcon,
