...
<template>
  ...
  <UiAccordion v-for="category in filteredAndSortedData" v-else ref="target" :key="category.id" type="single"
    class="w-full border-b last:border-b-0 flex flex-col gap-1 bg-white first:rounded-t-lg last:rounded-b-lg min-w-[700px]" collapsible>
    <UiAccordionItem v-slot="{ open }" :value="category.name" class="relative border-none">
      ...
      <UiAccordionContent :class="{ 'last:border-b': open }" class="pt-0 ml-0 pb-0 border-none">
        <CategoriesRecursive v-if="category.subcategory" :parent-id="category.id" :items="category.subcategory"
          :depth="1" />
      </UiAccordionContent>
    </UiAccordionItem>
  </UiAccordion>
