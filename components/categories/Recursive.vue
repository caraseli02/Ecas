...
const computedPadding = computed(() => {
    return BASE_PADDING + ((props.depth || 0) * LEVEL_PADDING);
});
</script>
...
<template>
    <template v-for="item in items" :key="item.uuid ?? item.localId">
        <section class="w-full border-none flex flex-col gap-1">
            <!-- Single item without subcategories -->
            <section v-if="item.subcategory?.length === 0"
                ...
                <article
                    ...
                    <div :style="{ 'padding-left': computedPadding + 'px' }" class="flex gap-2 max-w-[348px]">
                        <figure class="flex justify-center items-center min-w-10 h-10 rounded-lg bg-light-300">
                        ...
                        <p class="my-auto text-sm font-medium leading-4 text-neutral-700 max-md:max-w-full min-w-[260px] flex flex-col justify-between h-full">
                            {{ item.name }}
                            <span v-if="item.productCount" class="text-xs text-slate-500">
                        ...
                        <section class="w-full flex gap-2 items-center py-2 max-md:flex-wrap">
                            <article
                                ...
                                <div :style="{ 'padding-left': computedPadding + 'px' }" class="flex gap-2 max-w-[308px]">
                                    ...
                                    <p class="my-auto text-sm font-medium leading-4 text-neutral-700 max-md:max-w-full min-w-[260px] flex flex-col justify-between h-full">
                                        {{ item.name }}
                                        <span v-if="item.productCount" class="text-xs">
                                            {{ item.productCount }} items
                                        </span>
                                    </p>
                                </div>
