            ...
            <UiAccordion v-else type="multiple" class="w-full border-none flex flex-col gap-1" collapsible>
                <UiAccordionItem v-slot="{ open }" :value="item.name">
                    ...
                    <UiAccordionContent :class="{ 'last:border-b-[0.5px]': open }"
                        ...
                        <component :is="DomainsRecursive" v-if="item.subcategory && item.subcategory.length > 0"
                            :items="item.subcategory" :parent-id="parentId" :depth="(props.depth || 0) + 1" />
                                    ...
                                    <article
                                        ...
                                        <div :style="{ 'padding-left': computedPadding + 'px' }"
                                            ...
                                            <figure
                                                ...
                                                <div v-if="child.icon && child.icon !== 'N/A' && child.icon.includes('.svg')"
                                                    class="h-5 w-5 stroke-1" v-html="child.icon" />
