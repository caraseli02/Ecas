  ...
  <div class="flex gap-9 max-md:flex-col max-md:gap-0">
    <div class="flex flex-col w-6/12 max-md:ml-0 max-md:w-full">
      <div class="flex flex-col grow text-sm font-semibold leading-5 text-neutral-700 max-md:mt-9 max-md:max-w-full">
        <h2 class="text-base leading-7 max-md:max-w-full">{{ customerDetails.title }}</h2>
        <p class="mt-9 leading-6 max-md:max-w-full">{{ customerDetails.name }}</p>
    ...
    <div class="flex flex-col w-6/12 max-md:ml-0 max-md:w-full">
      <div class="flex flex-col max-md:mt-9 max-md:max-w-full">
        ...
        <div v-if="payType === 'Card'" class="flex gap-2 self-start pr-8 mt-9 max-md:pr-5">
...
loading="lazy"
        ...
        <UiDropdownMenu v-if="payType === 'Credit'">
            ...
            <article class="flex flex-col w-full max-w-[261px]">
              ...
              <section class="flex gap-5 justify-between mt-4 text-xs leading-5 text-gray-500 rounded-lg">
                <div class="flex flex-col justify-center">
                  <h3>Credit Limit</h3>
                  <p class="mt-1 font-medium text-neutral-800">{{ paymentMethod.creditInfo.creditLimit }}</p>
                  <h3 class="mt-4">Till Due</h3>
                  <p class="mt-1 font-medium text-neutral-800">{{ paymentMethod.creditInfo.tillDue }}</p>
                </div>
                <div class="flex flex-col justify-center">
                  <h3>Credit Term</h3>
                  <p class="mt-1 font-medium text-neutral-800">{{ paymentMethod.creditInfo.creditTerm }}</p>
                  <h3 class="mt-4">Total Spent</h3>
                  <p class="mt-1 font-medium text-neutral-800">{{ paymentMethod.creditInfo.totalSpent }}</p>
                </div>
                <div class="flex flex-col justify-center">
                  <h3>Due Date</h3>
                  <p class="mt-1 font-medium text-neutral-800">{{ paymentMethod.creditInfo.dueDate }}</p>
                  <h3 class="mt-4">Available</h3>
                  <p class="mt-1 font-medium text-red-500">{{ paymentMethod.creditInfo.available }}</p>
                </div>
              </section>
            </article>
          </UiDropdownMenuContent>
        </UiDropdownMenu>
        <UiDropdownMenu v-if="payType === 'Bank'">
          <UiDropdownMenuTrigger class="w-fit flex items-center gap-2 mt-9">
            Bank Transfer
            <InfoIcon class="w-4 h-4 stroke-1" />
          </UiDropdownMenuTrigger>
