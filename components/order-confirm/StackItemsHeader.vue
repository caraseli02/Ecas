...
<template>
  <section class="flex flex-col">
    <div class="flex justify-end md:justify-between flex-wrap gap-3 items-center w-full max-md:max-w-full">
      <UiPopover v-if="smAndLarger && isAdmin">
        <UiPopoverTrigger>
          <section class="flex gap-3 items-center text-sm leading-6 whitespace-nowrap text-zinc-800">
            <h2 class="self-stretch text-gray-500">
              AWB
            </h2>
            <InfoIcon
              v-if="isAdmin"
              class="object-contain shrink-0 self-stretch w-4 aspect-square text-slate-500"
            />
            <div
              v-if="!awb"
        ...
        <UiPopoverContent
          v-if="isAdmin"
          class="w-fit flex flex-col gap-2 p-2"
          align="end"
        >
          <UiButton
            v-if="!awb"
            class="hover:text-blue-500 justify-start gap-1"
            variant="ghost"
            size="sm"
            @click="generateAWB"
          >
      ...
      <div
        v-if="!isAdmin"
        class="hidden md:flex gap-3 items-center self-stretch text-sm leading-none"
      >
        <h4 class="self-stretch my-auto text-gray-500">
          Order Status
        </h4>
        <OrderConfirmStatusDisplay
          :status-color="getStatusByOrder().color"
          :status-text="getStatusByOrder().value"
        />
      </div>
      <section class="flex gap-2">
        <div
          v-if="isAdmin"
          class="flex gap-3 items-center self-stretch my-auto text-sm leading-none"
        >
          <h4 class="self-stretch my-auto text-gray-500">
            Order Status
          </h4>
          <OrderConfirmStatusSelect
            :statuses="statuses"
            :default-value="getStatusByOrder()"
            :order-id="order._id"
          />
        </div>
        <div class="flex gap-3 items-center self-stretch my-auto">
          ...
          <OrderConfirmActionMenu
            ...
            :order="order"
            @show-tracking-dialog="showTrackingDialog = true"
          />
        </div>
      </section>
    </div>
    ...
    >
      ...
      <UiPopover v-if="isAdmin && smallerThanSm">
        <UiPopoverTrigger class="w-full">
          <section class="flex justify-between w-full gap-3 items-center text-sm leading-6 whitespace-nowrap text-zinc-800">
            <h2 class="self-stretch my-auto text-gray-500">
              AWB
            </h2>
      ...
      <UiPopover>
        ...
        <UiPopoverContent
          class="w-fit flex flex-col gap-2 p-3 max-w-xs rounded-xl"
          align="end"
        >
          <article class="flex flex-col w-full">
            ...
            <div class="flex flex-col mt-3 w-full">
              ...
              <UiSeparator
                v-else
                class="bg-grey-100 mt-3"
              />
            </div>
            <div class="flex justify-end items-center">
              <UiButton
                class="w-fit gap-1 mt-1"
                variant="link"
              >
                View more
  ...
  <LazyOrderConfirmTrackingDialog
