<template>
  <div class="bg-white rounded-xl p-4 shadow-xs md:p-6">
    <div
      ...
      :class="[emptyData || error ? 'mb-4' : 'mb-6']"
    >
      ...
      >
        ...
        <Transition name="fade-full">
          ...
          >
            <button
              disabled
              class="flex items-center w-full text-left px-3 py-2 rounded-lg transition-colors duration-300 hover:bg-[#F2F2F2] hover:text-blue-500 disabled:cursor-not-allowed disabled:opacity-60 disabled:hover:bg-transparent disabled:hover:text-inherit"
            >
    ...
    >
      ...
      <SkeletonLoader
        v-if="isLoading"
        class="w-full h-16 md:h-11"
      />
      <div v-else>
        <div class="text-sm font-semibold mb-1">
          {{ shippingInformation?.find((x) => x.default)?.name1 || 'N/A' }}
        </div>
        <div class="text-sm">
          {{ shippingInformation?.find((x) => x.default)?.name2 || 'N/A' }}
        </div>
      </div>
    </div>
  </div>
