  ...
  <div class="flex flex-col bg-white rounded-xl p-4 shadow-xs md:p-6 xl:pt-10 xl:shadow-none xl:rounded-none">
    <div class="mb-4">
      <div class="flex items-start justify-between">
        ...
        <div
          v-else
          class="flex text-white bg-blue-500 text-xs leading-[1.66] font-semibold rounded-full px-2"
        >
          {{ customerDiscount + '%' }}
        </div>
      </div>
    </div>
    <p class="text-sm leading-[1.71] mb-9">
      Apply a customer discount (i.e. 5, 10, 17 etc).
    </p>
    <div class="mb-9">
      <SkeletonLoader
        v-if="loading"
      ...
      <template v-else>
        ...
        <div class="flex border border-border rounded-lg h-11 overflow-hidden">
          <div class="text-xl leading-[1.3] px-3 py-2 border-r border-border bg-[#F5F5F5]">
            %
          </div>
          <input
