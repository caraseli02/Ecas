  ...
  <div class="bg-white rounded-xl p-4 shadow-xs md:p-6 xl:order-1">
    ...
    <div class="grid grid-cols-1 gap-6 md:gap-9">
      <div class="border border-border rounded-xl p-4 md:p-6">
        <h3 class="leading-[1.5] font-semibold mb-4">
          Alerts and Notifications
        </h3>
        <p class="text-sm leading-[1.71] mb-8">
          Change to alerts and notification settings.
        </p>
        <div class="grid grid-cols-1 gap-6">
          <div class="grid grid-cols-[1fr,50px,50px]">
            <div />
            <div class="text-sm leading-[1.71] text-center">
              Email
            </div>
            <div class="text-sm leading-[1.71] text-right">
              App
            </div>
          </div>
          <div
            v-for="(item, index) in alertsAndNotifications"
            :key="index"
            class="grid grid-cols-[1fr,50px,50px]"
          >
            <div class="text-sm leading-[1.71] text-left">
              {{ item.label }}
            </div>
      ...
      <div class="border border-border rounded-xl p-4 md:p-6">
        ...
        <form
          class="md:grid md:items-end md:grid-cols-[1fr,auto] md:gap-4"
          @submit.prevent=""
        >
          <FormInput
      ...
      <div class="border border-border rounded-xl p-4 md:p-6">
        <h3 class="leading-[1.5] font-semibold mb-8">
          Marketing Preferences
        </h3>
        <div class="grid grid-cols-1 gap-6">
          <div
            v-for="(item, index) in marketingPreferences"
            :key="index"
