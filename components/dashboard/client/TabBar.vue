<template>
  <div class="relative flex items-center gap-8 bg-white rounded-xl px-6 shadow-xs overflow-x-auto hide-scrollbar">
    <button v-for="(filter, index) in orderFilters" :key="index"
      ...
      :class="[filter.label === activeOrderFilter.label ? 'text-blue-500' : 'hover:text-blue-500']" @click="
        activeOrderFilter = filter;
      ">
      <span v-if="filter.label">
        {{ filter.label }}
      </span>
      <span v-if="filter.total_items"
    ...
    <div class="absolute bottom-0 bg-blue-500 h-1 rounded-t-lg transition-all duration-300" :style="{
