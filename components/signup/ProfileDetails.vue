        ...
        <div class="mb-[30px]">
          <div class="grid grid-cols-1 gap-[15px] mb-10">
            ...
            <div class="bg-amber-400 px-3 py-1 rounded" v-if="firebaseToken">
              <small
                >You are logged in with Google, so you are not allowed to change
...
const useContactEmailCheck = () => {
  ...
  if (!firebaseToken) {
    if (
      ...
      details.value.accountEmail.value = details.value.useContactEmail
        ? contact.value.companyEmail.value
        : "";
      details.value.confirmAccountEmail.value = details.value.useContactEmail
        ? contact.value.companyEmail.value
        : "";
    } else {
      details.value.accountEmail.value = details.value.useContactEmail
