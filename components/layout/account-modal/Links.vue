<template>
  <div>
    <NuxtLink
v-show="!route.path.includes('dashboard')" :to="isAdmin ? '/dashboard' : 'dashboard/client'"
    ...
    <button
v-for="(menuItem, index) in menuActions" :key="index"
...
v-if="menuItem.text === 'Notifications' && unreadNotifications > 0"
    ...
    <NuxtLink
v-for="(menuItem, index) in menuLinks" :key="index" :to="menuItem.to"
