  ...
  <div class="flex flex-col pb-4" :class="[inModal ? '' : 'border-b border-gray-200 mx-2 last:border-b-0']">
    ...
    <div v-if="!inModal" class="flex items-end justify-between">
      ...
      <QuantityButtons
          ...
          :object="{action : ProductAction.Update, id: productItem.id, min : quantity} as ProductActionObject"
          class="mr-10"
      />
    </div>
  </div>
  <Teleport to="body">
    <Transition name="slide-from-top">
      <LayoutFavoritesModalsDelete
          v-if="deleteItem" :products="[productItem]" :delete-from-cart="true"
          @close="deleteItem = false"/>
    </Transition>
    <Transition name="slide-from-top">
      <LayoutFavoritesModalsCopyMoveItems
          v-if="copyItems" :items="[productItem]" action="copy"
