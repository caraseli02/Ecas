<template>
  <div ref="elDOM" class="fixed z-[60] top-0 left-0 w-full h-screen pointer-events-none">
    <div
        class="relative z-10 flex flex-col ml-auto w-full h-full bg-white max-h-vh overflow-hidden pointer-events-auto md:w-[440px]">
      <div
          class="flex flex-col"
          :style="{
      ...
      >
        ...
        <div class="relative z-10 flex items-center gap-6 px-4 bg-white md:px-6">
          <button
              ...
              :class="[activeNavItem === textUtil.slugify(item.label) ? 'text-blue-500' : 'text-dark after:opacity-0']"
      ...
      <div class="flex-1 h-[calc(100%-102px)]">
        ...
        <Cart v-else-if="activeNavItem === 'shopping-cart'" :data="data"/>
      </div>
    </div>
  </div>
</template>
