    ...
    <UiPopover v-model:open="open">
        <UiPopoverTrigger as-child>
            <section class="flex flex-col gap-2 w-fit">
                <div class="text-sm font-medium leading-4 text-zinc-700">{{ title }}</div>
                <UiButton class="w-fit h-fit p-0" variant="ghost">
                    <div
                        class="flex flex-col justify-center items-center h-16 w-16 max-w-16 bg-white rounded-lg border border-gray-200 border-solid shadow-sm max-md:px-5"
                    >
        ...
        <UiPopoverContent :collision-padding="10" class="w-[300px] p-0 flex flex-col gap-4">
            ...
            <div :key="search" class="flex content-start flex-wrap gap-2 px-2 pb-4 h-[200px] overflow-y-auto">
                <UiButton v-for="icon in filteredIcons" :key="icon" variant="ghost" size="icon" @click="selectIcon(icon)">
                    <IconAsync :name="icon" class="h-6 w-6 stroke-1" />
                </UiButton>
            </div>
        </UiPopoverContent>
    </UiPopover>
