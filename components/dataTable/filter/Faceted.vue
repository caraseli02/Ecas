...
import type { OrderStatus } from '~/types/order-summary/item';
...
interface DataTableFacetedFilter {
  ...
  statusColors?: {
    [value in OrderStatus | AccountType]: string; // Index signature
  };
}
  ...
  <UiPopover class="">
    <UiPopoverTrigger as-child>
      <UiButton
        variant="outline"
        size="sm"
        class="!ml-0 border-dashed text-neutral-700 hover:bg-light-300"
      >
          ...
          <div class="hidden space-x-1 lg:flex">
            ...
            <template v-else>
              ...
              <template v-if="statusColors">
                <UiBadge
                  v-for="option in props.column?.getFilterValue().filter((option) => selectedValues.has(option))"
                  :key="option"
                  variant="secondary"
                  class="rounded bg-light-300 px-1 py-[3px] font-normal text-neutral-700"
                >
                  <span
                    :class="statusColors[option]"
                    class="h-3 w-3 mr-2 rounded-full text-xs flex justify-center items-center"
                  />
                  {{ option }}
                </UiBadge>
      ...
      <UiCommand>
        ...
        <UiCommandList>
          ...
          <UiCommandGroup>
            ...
            <template v-if="statusColors">
              <UiCommandItem
                v-for="(color, value) in statusColors"
                :key="value"
                :value="value"
                @select="
                  () => {
